...
// Main function to process the JSON data and calculate MPR, ANOVA, and perform Taxonomy Analysis
function processFile(filePath) {
  const data = readJSONFile(filePath);
  const { neighborRanges, nodes, edges } = data;
  ...
  neighborRanges.forEach((range) => {
    ...
    // Put everything into a single object for this range
    result.ranges.push({
...
function main() {
  const filePath = './crime-0.json'; // Path to your input JSON file
  const result = processFile(filePath);
  writeResultsToFile(filePath, result);
}
